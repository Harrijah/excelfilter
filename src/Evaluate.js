...
function Evaluate() {
  ...
  const MY_API_KEY01 = process.env.REACT_APP_MY_API_KEY01;
  ...
  const analyzeUrls = async () => {
      ...
      if (keywords.some((k) => k)) {
        for (let j = 0; j < keywords.length; j++) {
          ...
          try {
            ...
            console.error(
              `Erreur lors de la recherche pour le mot-clé ${j + 1} :`,
              error
            );
    ...
    // Ajouter les colonnes des mots-clés dans les en-têtes
    const keywordHeaders = keywords.filter((k) => k);
    setHeaders((prev) => [...prev, ...keywordHeaders]);
  };
  ...
  return (
    <div className="container">
      <Navigation />
      <div className="app">
        <h1>Évaluation des comptes Salesforce</h1>
        ...
        <div className="filters">
          ...
          <div className="filter-row thresholds">
            <label>Seuils de capacité :</label>
            <input
              type="number"
              value={threshold1}
          ...
          {/* Ajouter des inputs pour les mots-clés */}
